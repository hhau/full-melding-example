...
The multi-stage Metropolis-Hastings MCMC approach of @goudie:etal:18 necessitates an order in which we consider the submodels.
Here, there is a natural ordering due to the non-invertible link function used to compute $\phi_{a}$.
In general melding is mathematically insensitive to this ordering, as any link function can be extended into an invertible one without affecting the outcome.
...
```{r psi_trace_wsre, fig.cap = "$\\psi_{2}$ trace, final 1000 samples. Mixing is governed by the mixing of $\\phi$."}
knitr::include_graphics("plots/stage-two-wsre-psi-trace.pdf")
```

### Traceplots of $\phi_{2}$ __without__ `wsre`

```{r phi_trace_no_wsre, fig.cap = "$\\phi$ trace without the self-density ratio estimate."}
knitr::include_graphics("plots/stage-two-no-wsre-phi-trace.pdf")
```

$\psi_{2}$ trace:

```{r psi_trace_no_wsre, fig.cap = "$\\psi_{2}$ trace, final 1000 samples, no self-density estimate."}
knitr::include_graphics("plots/stage-two-no-wsre-psi-trace.pdf")
```
